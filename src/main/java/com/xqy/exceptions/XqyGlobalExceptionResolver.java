...
/**
 * @Author xu
 * @Date 2023/9/30 13:50
 * @Version 1.0
 * 全局异常处理
 * @ResponseBody响应内容为json数据，反之则是响应内容为页面
 */
@Component
public class XqyGlobalExceptionResolver implements HandlerExceptionResolver {
    @Override
    public ModelAndView resolveException(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Object handler, Exception e) {
        ...
        if (e instanceof XqyUnLoginException) {
        ...
        if (handler instanceof HandlerMethod) {
            ...
            if (responseBody == null) {
                //如果为视图
                //判断是否为参数异常
                if (e instanceof XqyParamsException) {
                    XqyParamsException PE = (XqyParamsException) e;
                    mv.addObject("msg", PE.getMsg());
                    mv.addObject("code", PE.getCode());
                }
                //是否为认证异常
                else if (e instanceof XqyAuthException) {
                    XqyAuthException ae = (XqyAuthException) e;
